...
function RegistrarContent() {
  ...
  async function handleSubmit() {
    ...
    const result = await registerResult(betId, legsPayload, legs, Number(bet.odds_combined), Number(bet.stake))
          ...
          <div>
            <section className="rounded-3xl border border-black/15 bg-white p-5 mb-4">
              <p className="text-xs font-bold uppercase tracking-wider text-[#4B5563] mb-1">
                {bet.type} - {legs.length} {legs.length === 1 ? 'evento' : 'eventos'}
              </p>
                  ...
                  <section key={leg.id} className="rounded-3xl border border-black/15 bg-white p-5">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <button
                        ...
                        className={'rounded-xl py-2.5 text-xs font-bold border-2 ' + (currentStatus === 'WIN' ? 'bg-[#10B981] text-white border-[#10B981]' : 'bg-white text-[#1F2937] border-black/15')}
                      >
                        Gano
                      </button>
                      <button
                        onClick={function () { setStatus(leg.id, 'LOSS') }}
                        className={'rounded-xl py-2.5 text-xs font-bold border-2 ' + (currentStatus === 'LOSS' ? 'bg-[#FF7A8C] text-white border-[#FF7A8C]' : 'bg-white text-[#1F2937] border-black/15')}
                      >
                        Perdio
                      </button>
                      <button
                        onClick={function () { setStatus(leg.id, 'VOID') }}
                        className={'rounded-xl py-2.5 text-xs font-bold border-2 ' + (currentStatus === 'VOID' ? 'bg-[#9CA3AF] text-white border-[#9CA3AF]' : 'bg-white text-[#1F2937] border-black/15')}
                      >
                        Anulado
                      </button>
                    </div>
            ...
            {preview && (
              <section className="rounded-3xl p-6 mb-4" style={{ backgroundColor: '#1F2937' }}>
                ...
                <p className="font-display text-3xl font-extrabold" style={{ color: preview.profit >= 0 ? '#10B981' : '#FF7A8C' }}>
                  {preview.status} {formatUnits(preview.profit)}
                </p>
